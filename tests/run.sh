...
if grep -q "./myOS shell ./tests/test1.txt" "./tests/processes.txt" && grep -q "sleep 1" "./tests/processes.txt" ; then echo "Test 1 Succeeded" >> report.txt; else echo "Test 1 Failed" >> report.txt; fi
